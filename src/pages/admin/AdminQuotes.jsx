// pages/admin/AdminQuotes.jsx
// Quote Generator — create, edit, list and print/download quotes for
// students/clients. Quotes are persisted through the backend (see
// server/index.js `/api/admin/quotes*` + server/store.js), the same
// pattern used by every other admin resource in this app.
import React, { useEffect, useMemo, useState } from 'react'
import {
  FileSignature, Plus, Trash2, Printer, Download, Pencil, X, Search,
...
const STATUS_STYLES = {
  ...
  accepted: { label: 'Accepted', cls: 'text-green-600 bg-green-50', icon: CheckCircle2 },
  declined: { label: 'Declined', cls: 'text-red-600 bg-red-50', icon: AlertCircle },
  expired: { label: 'Expired', cls: 'text-orange-600 bg-orange-50', icon: AlertCircle },
}
...
const emptyQuote = () => ({
  from: safeParse('ecsprep_quote_from') || {
...
function calcTotals(quote) {
  ...
  const discountAmount = subtotal * ((Number(quote.discount) || 0) / 100)
  const taxable = subtotal - discountAmount
  const taxAmount = taxable * ((Number(quote.taxRate) || 0) / 100)
...
export default function AdminQuotes() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const { quotes: list } = await apiRequest('/api/admin/quotes')
      setQuotes(list)
  ...
  const saveQuote = async (statusOverride) => {
    ...
    localStorage.setItem('ecsprep_quote_from', JSON.stringify(form.from))
    setSaving(true)
    try {
      const payload = { ...form, status: statusOverride || form.status }
      if (editingId) {
        const { quote } = await apiRequest(`/api/admin/quotes/${editingId}`, { method: 'PATCH', body: payload })
        setQuotes(list => list.map(i => (i.id === quote.id ? quote : i)))
      } else {
        const { quote } = await apiRequest('/api/admin/quotes', { method: 'POST', body: payload })
        setQuotes(list => [quote, ...list])
  ...
  const deleteQuote = async (id) => {
    ...
    try {
      await apiRequest(`/api/admin/quotes/${id}`, { method: 'DELETE' })
      setQuotes(list => list.filter(i => i.id !== id))
    } catch (e) { alert(e.message) }
  }

  const printQuote = (inv) => {
    startEdit(inv)
    // Wait for the builder (and its print view) to render, then open the
    // browser's print dialog — the .quote-print-area is the only thing
    // visible thanks to the @media print rules below, so "Save as PDF" in
    // the print dialog produces a clean quote PDF with no extra setup.
    ...
    <div className="max-w-6xl mx-auto">
      <style>{`
        @media print {
          ...
          .quote-print-area { position: absolute; top: 0; left: 0; width: 100%; padding: 24px; }
      ...
      {view === 'list' ? (
        <>
          <div className="flex items-center justify-between mb-6 no-print">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><FileSignature size={24} /> Quote Generator</h1>
              <p className="text-gray-500 text-sm mt-0.5">Create, manage and send price quotes to prospective clients</p>
            </div>
            <button
              onClick={startNew}
          ...
          {loading ? (
            <div className="text-center py-12 text-gray-400 no-print">Loading quotes…</div>
          ) : filtered.length === 0 ? (
                  ...
                  {filtered.map((inv) => {
                    ...
                    return (
                      <tr key={inv.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-5 py-3 text-gray-500">{inv.validUntil ? new Date(inv.validUntil).toLocaleDateString('en-GB') : '—'}</td>
                        <td className="px-5 py-3">
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => printQuote(inv)} title="Print / Save as PDF" className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Printer size={16} /></button>
                            ...
                            <button onClick={() => deleteQuote(inv.id)} title="Delete" className="p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    )
        ...
        <>
          ...
          <div className="flex items-center justify-between mb-6 no-print">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => saveQuote('draft')} disabled={saving} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors disabled:opacity-50">
                <Save size={15} /> Save Draft
              </button>
              <button onClick={() => saveQuote('sent')} disabled={saving} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-blue-500 to-secondary rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 space-y-4 no-print">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                <h3 className="font-semibold text-gray-900 mb-3">Quote For</h3>
                <div className="space-y-2">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                ...
                <div className="space-y-2">
                  <label className="block text-xs text-gray-500">Valid until</label>
                  <input type="date" value={form.validUntil} onChange={(e) => setForm(f => ({ ...f, validUntil: e.target.value }))} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  ...
                  <textarea value={form.notes} onChange={(e) => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Terms & conditions, validity notes, etc." rows={3} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            </div>

            {/* Live preview / printable quote */}
            <div className="lg:col-span-2">
              <div className="quote-print-area bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-8">
                <div className="flex items-start justify-between mb-8">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm text-gray-500 mt-1">{editingId ? quotes.find(i => i.id === editingId)?.number : 'Draft — number on save'}</p>
                    {form.validUntil && <p className="text-sm text-gray-500">Valid until {new Date(form.validUntil).toLocaleDateString('en-GB')}</p>}
                ...
                <div className="mb-6">
                  <p className="text-xs uppercase tracking-wide text-gray-400 font-semibold mb-1">Quote For</p>
