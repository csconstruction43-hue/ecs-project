// pages/ecs-cards/ProfessionallyQualifiedPersonPqpPage.jsx — dedicated ECS card page for "Professionally Qualified Person (PQP)"
...
import {
  ...
  ArrowRight, Zap, BarChart3, CreditCard, GraduationCap,
} from 'lucide-react'
import Seo, { faqSchema, breadcrumbSchema } from '../../components/Seo'
import { CARD_CONTENT } from '../../data/ecsCardContent'

const CARD = {
  name: `Professionally Qualified Person (PQP)`,
  slug: 'professionally-qualified-person-pqp',
  desc: `For members of approved professional institutions and bodies.`,
  path: '/ecs-black-card-mock-test',
  category: 'management',
}
const CATEGORY = { id: 'management', name: `Site Management`, desc: `Supervisory and management routes for running sites and contracts.` }
const META = {
...
const faqs = [
    { q: `Do I need to sit a test for the Professionally Qualified Person (PQP) card?`, a: `Yes. This route sits behind the MAP HS&E test, the same core HS&E assessment ECSPrep's mock tests are built to prepare you for.` },
    { q: `How long does the Professionally Qualified Person (PQP) card last?`, a: `5 years, in line with the Black / White Card (Manager) it belongs to. Always confirm the current rule for your exact route at ecscard.org.uk.` },
    ...
    { q: `Is Professionally Qualified Person (PQP) the right card for my role?`, a: `It's one of several routes under Site Management. If you're not sure, browse the other cards in this category below or use the card finder on our ECS Cards page.` },
    ...(ABOUT?.extraFaqs || []),
]
...
export default function ProfessionallyQualifiedPersonPqpPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      {/* Related cards */}
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
        <h2 className="text-xl font-bold text-gray-900 mb-1">Other cards in Site Management</h2>
        <p className="text-gray-500 text-sm mb-6">Not quite the right fit? These sit in the same occupational group.</p>
        <div className="grid sm:grid-cols-3 gap-4">
            <Link
              to="/ecs-cards/site-supervisor"
            ...
            >
              ...
              <div>
                <div className="text-sm font-semibold text-gray-900">Site Supervisor</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Supervisory-level card for overseeing electrotechnical work on site.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/site-manager"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">Site Manager</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Site management card for leading projects and delivery on site.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/contracts-manager"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">Contracts Manager</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Senior card route for managing contracts and commercial delivery.</p>
              </div>
            </Link>
        </div>
      </section>
