// pages/ecs-cards/BuildingControlsInstallerEngineerPage.jsx — dedicated ECS card page for "Building Controls Installer / Engineer"
...
import {
  ...
  ArrowRight, Zap, Flame, CreditCard, GraduationCap,
} from 'lucide-react'
import Seo, { faqSchema, breadcrumbSchema } from '../../components/Seo'
import { CARD_CONTENT } from '../../data/ecsCardContent'

const CARD = {
  name: `Building Controls Installer / Engineer`,
  slug: 'building-controls-installer-engineer',
  desc: `For specialists installing building controls and automation systems.`,
  path: '/ecs-skilled-worker-test',
  category: 'fess',
}
const CATEGORY = { id: 'fess', name: `Fire, Emergency & Security Systems (FESS)`, desc: `Fire alarm, emergency lighting and security system installers and engineers.` }
const META = {
...
const faqs = [
    { q: `Do I need to sit a test for the Building Controls Installer / Engineer card?`, a: `Yes. This route sits behind the Operatives or Specialist HS&E test, the same core HS&E assessment ECSPrep's mock tests are built to prepare you for.` },
    { q: `How long does the Building Controls Installer / Engineer card last?`, a: `5 years (1 year on the temporary Experienced Worker route), in line with the Blue / Gold Card (Skilled Worker) it belongs to. Always confirm the current rule for your exact route at ecscard.org.uk.` },
    ...
    { q: `Is Building Controls Installer / Engineer the right card for my role?`, a: `It's one of several routes under Fire, Emergency & Security Systems (FESS). If you're not sure, browse the other cards in this category below or use the card finder on our ECS Cards page.` },
    ...(ABOUT?.extraFaqs || []),
]
...
export default function BuildingControlsInstallerEngineerPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      {/* Related cards */}
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
        <h2 className="text-xl font-bold text-gray-900 mb-1">Other cards in Fire, Emergency & Security Systems (FESS)</h2>
        <p className="text-gray-500 text-sm mb-6">Not quite the right fit? These sit in the same occupational group.</p>
        <div className="grid sm:grid-cols-3 gap-4">
            <Link
              to="/ecs-cards/fess-apprentice"
            ...
            >
              ...
              <div>
                <div className="text-sm font-semibold text-gray-900">FESS Apprentice</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">For apprentices training in Fire, Emergency & Security Systems.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/fess-labourer"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">FESS Labourer</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Entry-level labouring card within the FESS occupational structure.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/fess-systems-operative"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">FESS Systems Operative</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Technical operative card for FESS installation and servicing work.</p>
              </div>
            </Link>
        </div>
      </section>
