// pages/ecs-cards/CellularNetworkFieldEngineerPage.jsx — dedicated ECS card page for "Cellular Network Field Engineer"
...
import {
  ...
  ArrowRight, Zap, Radio, CreditCard, GraduationCap,
} from 'lucide-react'
import Seo, { faqSchema, breadcrumbSchema } from '../../components/Seo'
import { CARD_CONTENT } from '../../data/ecsCardContent'

const CARD = {
  name: `Cellular Network Field Engineer`,
  slug: 'cellular-network-field-engineer',
  desc: `Field engineer card for mobile and cellular network work.`,
  path: '/ecs-skilled-worker-test',
  category: 'network',
}
const CATEGORY = { id: 'network', name: `Network & Telecoms`, desc: `Structured cabling, telecoms and mobile network infrastructure roles.` }
const META = {
...
const faqs = [
    { q: `Do I need to sit a test for the Cellular Network Field Engineer card?`, a: `Yes. This route sits behind the Operatives or Specialist HS&E test, the same core HS&E assessment ECSPrep's mock tests are built to prepare you for.` },
    { q: `How long does the Cellular Network Field Engineer card last?`, a: `5 years (1 year on the temporary Experienced Worker route), in line with the Blue / Gold Card (Skilled Worker) it belongs to. Always confirm the current rule for your exact route at ecscard.org.uk.` },
    ...
    { q: `Is Cellular Network Field Engineer the right card for my role?`, a: `It's one of several routes under Network & Telecoms. If you're not sure, browse the other cards in this category below or use the card finder on our ECS Cards page.` },
    ...(ABOUT?.extraFaqs || []),
]
...
export default function CellularNetworkFieldEngineerPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      {/* Related cards */}
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
        <h2 className="text-xl font-bold text-gray-900 mb-1">Other cards in Network & Telecoms</h2>
        <p className="text-gray-500 text-sm mb-6">Not quite the right fit? These sit in the same occupational group.</p>
        <div className="grid sm:grid-cols-3 gap-4">
            <Link
              to="/ecs-cards/network-infrastructure-assistant"
            ...
            >
              ...
              <div>
                <div className="text-sm font-semibold text-gray-900">Network Infrastructure Assistant</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Assistant-level card for network and structured cabling work.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/network-infrastructure-installer"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">Network Infrastructure Installer</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Installer and technician levels for network infrastructure routes.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/lv-jointer"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">LV Jointer</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Low-voltage jointer working on LV distribution networks.</p>
              </div>
            </Link>
        </div>
      </section>
