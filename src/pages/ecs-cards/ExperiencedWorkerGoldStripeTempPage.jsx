// pages/ecs-cards/ExperiencedWorkerGoldStripeTempPage.jsx — dedicated ECS card page for "Experienced Worker (Gold Stripe – Temp)"
...
const CARD = {
  name: `Experienced Worker (Gold Stripe – Temp)`,
  slug: 'experienced-worker-gold-stripe-temp',
  desc: `Temporary route for experienced workers progressing towards a full card.`,
  path: '/ecs-skilled-worker-test',
  category: 'entry',
}
const CATEGORY = { id: 'entry', name: `Entry & General Routes`, desc: `Starting points on site before you specialise into a trade.` }
const META = {
...
const faqs = [
    { q: `Do I need to sit a test for the Experienced Worker (Gold Stripe – Temp) card?`, a: `Yes. This route sits behind the Operatives or Specialist HS&E test, the same core HS&E assessment ECSPrep's mock tests are built to prepare you for.` },
    { q: `How long does the Experienced Worker (Gold Stripe – Temp) card last?`, a: `5 years (1 year on the temporary Experienced Worker route), in line with the Blue / Gold Card (Skilled Worker) it belongs to. Always confirm the current rule for your exact route at ecscard.org.uk.` },
    ...
    { q: `Is Experienced Worker (Gold Stripe – Temp) the right card for my role?`, a: `It's one of several routes under Entry & General Routes. If you're not sure, browse the other cards in this category below or use the card finder on our ECS Cards page.` },
    ...(ABOUT?.extraFaqs || []),
]
...
export default function ExperiencedWorkerGoldStripeTempPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      {/* Related cards */}
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
        <h2 className="text-xl font-bold text-gray-900 mb-1">Other cards in Entry & General Routes</h2>
        <p className="text-gray-500 text-sm mb-6">Not quite the right fit? These sit in the same occupational group.</p>
        <div className="grid sm:grid-cols-3 gap-4">
            <Link
              to="/ecs-cards/labourer"
            ...
            >
              ...
              <div>
                <div className="text-sm font-semibold text-gray-900">Labourer</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Entry-level route for general site support and basic labouring duties.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/electrical-labourer"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">Electrical Labourer</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">For workers assisting qualified electricians on electrical sites.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/apprentice"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">Apprentice</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">For anyone on a recognised electrical apprenticeship framework.</p>
              </div>
            </Link>
        </div>
      </section>
