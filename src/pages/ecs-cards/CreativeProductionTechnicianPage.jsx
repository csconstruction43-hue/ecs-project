// pages/ecs-cards/CreativeProductionTechnicianPage.jsx — dedicated ECS card page for "Creative Production Technician"
...
import {
  ...
  ArrowRight, Zap, FileText, CreditCard, GraduationCap,
} from 'lucide-react'
import Seo, { faqSchema, breadcrumbSchema } from '../../components/Seo'
import { CARD_CONTENT } from '../../data/ecsCardContent'

const CARD = {
  name: `Creative Production Technician`,
  slug: 'creative-production-technician',
  desc: `Technical role supporting creative and live event production.`,
  path: '/ecs-skilled-worker-test',
  category: 'av',
}
const CATEGORY = { id: 'av', name: `AV, Broadcast & Creative Production`, desc: `Audio-visual, broadcast and live event technical roles.` }
const META = {
...
const faqs = [
    { q: `Do I need to sit a test for the Creative Production Technician card?`, a: `Yes. This route sits behind the Operatives or Specialist HS&E test, the same core HS&E assessment ECSPrep's mock tests are built to prepare you for.` },
    { q: `How long does the Creative Production Technician card last?`, a: `5 years (1 year on the temporary Experienced Worker route), in line with the Blue / Gold Card (Skilled Worker) it belongs to. Always confirm the current rule for your exact route at ecscard.org.uk.` },
    ...
    { q: `Is Creative Production Technician the right card for my role?`, a: `It's one of several routes under AV, Broadcast & Creative Production. If you're not sure, browse the other cards in this category below or use the card finder on our ECS Cards page.` },
    ...(ABOUT?.extraFaqs || []),
]
...
export default function CreativeProductionTechnicianPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      {/* Related cards */}
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
        <h2 className="text-xl font-bold text-gray-900 mb-1">Other cards in AV, Broadcast & Creative Production</h2>
        <p className="text-gray-500 text-sm mb-6">Not quite the right fit? These sit in the same occupational group.</p>
        <div className="grid sm:grid-cols-3 gap-4">
            <Link
              to="/ecs-cards/av-operative-av-technician"
            ...
            >
              ...
              <div>
                <div className="text-sm font-semibold text-gray-900">AV Operative / AV Technician</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Audio-visual technician and operative roles across venues and events.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/broadcast-and-media-supervisor"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">Broadcast & Media Supervisor</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Supervisory card for broadcast and media technical operations.</p>
              </div>
            </Link>

            <Link
              to="/ecs-cards/creative-production-operative"
              className="flex items-start gap-3 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              <ClipboardCheck size={16} className="text-blue-500 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-gray-900">Creative Production Operative</div>
                <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">Supporting technical work in stage, theatre and live events.</p>
              </div>
            </Link>
        </div>
      </section>
